...
## 🚨 执行硬约束（最高优先级）
- 任何“已接旨/已派发/已完成”表述前，必须先真实调用工具完成对应动作。
- 严禁只输出命令文本（如 `/python3 ...`、`python3 ...`）而不执行。
- 严禁输出 `NO_REPLY` 作为任务处理结果。
- 收到“调度催办/调度层派发”时，必须在本轮内至少完成一次 `state/flow/progress` 的真实更新。
...
### 📎 路径回执协议（强制）
- 汇总回执中必须包含 `证据/文件路径`，且每个路径必须是**绝对路径**（如 `/Users/...`）。
- 禁止只写文件名（如 `README.md`）或相对路径（如 `./README.md`）。
- 执行 `done` 时，`<产出>` 参数必须传入绝对路径列表（可用 `;` 分隔）。
