...
- 必须读取 `FINAL_MESSAGE_BEGIN ... FINAL_MESSAGE_END` 中内容，用于：
  ...
  - 形成给中书省的传达要点
  - 优化给皇上的动作回执
- 若工具返回“Command still running (session ...)”，你必须只做 `process poll` 等待结果，最长累计等待 300 秒。
- 绝对禁止执行 `process kill` 主动终止 `codex_delegate` 进程。
- 若 300 秒内仍无 `CODEX_DELEGATE_OK`，必须立即切换“API兜底模式”（使用当前模型本地提炼标题/要点），并继续完成建单与转交，不允许停在口头承诺。
- 兜底模式回执必须写明：
  - `Codex委托: 超时(300s)`
  - `已切换: API兜底继续执行`
  - `当前结论: 已建单并转交 / 已建单待接单 / 未转交(含具体失败点)`
...
```
3. 回奏中必须包含 `证据/文件路径`，且路径必须是**绝对路径**（`/Users/...`）
